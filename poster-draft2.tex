...
\end{equation}
where $v$ is the velocity of the soliton and $\phi$ is a phase factor. Defining $\zeta$ in this way results in dimensionless probabilities which integrate to unity over $(-\infty,\infty)$.
Substituting the $v=0$ case into the time-independent Schrodinger equation and setting E to be $\zeta^2$ to eliminate $x$ yields a stationary-state value for the interaction parameter of $g=-4\zeta$.
...
{\fontsize{16pt}{20pt}\selectfont
To investigate whether the extent and parameters used here are well explained by a particle model, an effective inter-soliton potential will be introduced within Newtonian dynamics and the solitons treated as classical particles. 
